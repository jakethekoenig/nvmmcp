/**
 * Implementation of the nvim_user_view resource
 * Provides information about the current state of Neovim buffers
 */
...
/**
 * Get the current state of Neovim buffers
 * Shows visible buffers, cursor positions, tab information, etc.
 */
export async function getNvimUserView() {
  try {
    const nvim = getNvim();
...
${formattedResult || "No visible buffers found"}`;
    ...
    if (error instanceof Error && error.message && error.message.includes('Timeout')) {
      console.error(`Timeout error in nvim_user_view resource: ${error.message}`);
      return `Error: Neovim RPC operation timed out after ${NVIM_RPC_TIMEOUT_MS}ms.\n` +
             `The Neovim process may have been closed or become unresponsive.\n` +
             `Please check if Neovim is still running and listening.`;
    }
    
    // Handle other errors
    return `Error in nvim_user_view resource: ${error}`;
  }
}
